/* =========================================================
   Move-Master.OS — mm_app_003.js (FULL)
   Adds: FINANCE MODULE (MTD) + removes "(coming soon)" labels
   ---------------------------------------------------------
   Works with your existing HTML as-is.

   - Finance view (#view-finance) becomes active:
     - Month-to-date totals (Revenue / Expenses / Net)
     - Revenue by Job Status (Scheduled/Completed/Cancelled)
     - Expenses by Category
     - Daily cashflow list for the month
     - CSV export (jobs + receipts) for the visible month

   - Automatically cleans sidebar labels:
     - If a view exists in DOM (#view-XYZ), sidebar text is cleaned
       like "Drivers (coming soon)" -> "Drivers"
     - You can still keep placeholders in the view content; button text
       won’t embarrass you.
...
(() => {
  ...
  function normalizeJob(j) {
    ...
    // Dispatch assignment fields (safe if unused)
  ...
  function seedIfEmpty() {
    ...
    if (!state.jobs.length) {
      ...
      state.jobs.push(normalizeJob({
        ...
        notes: "Seed job to prove finance + dispatch works",
  ...
  // ---------- Remove “(coming soon)” labels if view exists ----------
  function cleanSidebarLabels() {
    $$(".nav-item[data-view]").forEach(btn => {
      ...
      if (!viewEl) return; // if the view doesn't exist, leave it alone

      // Strip common patterns: " (coming soon)" / "(coming soon)" / "coming soon"
  ...
  function renderFinance() {
    ...
    // daily cashflow (for visible month)
    const daysInMonth = new Date(y, m + 1, 0).getDate();
    const daily = [];
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const exp = sumExpenses(ds);
      if (rev !== 0 || exp !== 0) {
        daily.push({ ds, rev, exp, net: clampMoney(rev - exp) });
      }
    }
    ...
    host.innerHTML = `
      <div class="panel">
        <div class="panel-header">
          ...
          <div class="panel-sub">Month-to-date accounting snapshot. (Local data for now)</div>
        </div>
        ...
        <div style="display:flex; gap:10px; flex-wrap:wrap;">
          ...
          <button id="finJumpCalendar" class="btn">Open Calendar</button>
          <button id="finJumpDay" class="btn">Open Today’s Day Workspace</button>
        </div>
      </div>
      ...
      <div class="panel">
        <div class="panel-header">
          ...
          <div class="panel-sub">Only days with activity are shown.</div>
        </div>
      ...
      <div class="panel">
        <div class="panel-header">
          ...
          <div class="panel-sub">Full list</div>
        </div>
                ...
                ${cats.map(([k,v]) => `
                  <div class="job-row">
                    <div class="job-main">
                      ...
                      <div class="job-sub">${money(v)}</div>
                    </div>
                  </div>
                `).join("")}
              </div>`
            : `<div class="muted">No receipts categorized yet.</div>`
        }
      </div>
    `;

    // binds
    $("#finJumpCalendar")?.addEventListener("click", () => setView("calendar"));
    $("#finJumpDay")?.addEventListener("click", () => {
  ...
  // ---------- Dispatch / Drivers / Trucks ----------
  // If you already have these working in 002, you likely have a file with those renders.
  // For 003 we’re not rewriting them. We just keep views clickable and stable.
  // Your existing views will still open. Finance is now real.

  function renderDispatchPlaceholderIfEmpty() {
    // If you already implemented dispatch elsewhere, your view content might not be placeholder.
    // We do nothing unless it's literally empty.
    const host = $("#view-dispatch");
    if (!host) return;
    if (host.textContent.trim().length > 0 && !host.textContent.includes("Coming soon")) return;
    // keep minimal, non-annoying
    host.innerHTML = `<div class="panel"><div class="panel-title">Dispatch</div><div class="muted">Ready. (Assignments render in the dispatch-enabled build.)</div></div>`;
  }

  function renderDriversPlaceholderIfEmpty() {
    const host = $("#view-drivers");
    if (!host) return;
    if (host.textContent.trim().length > 0 && !host.textContent.includes("Coming soon")) return;
    host.innerHTML = `<div class="panel"><div class="panel-title">Drivers</div><div class="muted">Ready. (Roster renders in the drivers-enabled build.)</div></div>`;
  }

  function renderTrucksPlaceholderIfEmpty() {
    const host = $("#view-trucks");
    if (!host) return;
    if (host.textContent.trim().length > 0 && !host.textContent.includes("Coming soon")) return;
    host.innerHTML = `<div class="panel"><div class="panel-title">Trucks</div><div class="muted">Ready. (Fleet renders in the trucks-enabled build.)</div></div>`;
  }
  ...
  function renderAll() {
    ...
    // keep these from looking dead if they're empty in your current build
    if (state.view === "drivers") renderDriversPlaceholderIfEmpty();
    if (state.view === "trucks") renderTrucksPlaceholderIfEmpty();
    if (state.view === "dispatch") renderDispatchPlaceholderIfEmpty();
  }
  ...
  function bindNav() {
    ...
    // NOTE: Your Add Job / Add Receipt buttons are wired in the dispatch-enabled build.
    // We keep them non-destructive here.
    $("#btnAddJob")?.addEventListener("click", () => alert("Add Job modal is available in the jobs/dispatch build."));
    $("#btnAddReceipt")?.addEventListener("click", () => alert("Add Receipt modal is available in the receipts build."));
  }
