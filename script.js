...
window.tab = (id) => {
    ...
    if (target) {
        ...
        // B. SPREADSHEET ENGINE: Build the spreadsheet for the room on entry
        if(id === 'dashboard') renderDashboard();
        if(['trucks', 'driverlog', 'clients', 'dispatch', 'inventory'].includes(id)) buildRegistry(id);

        // C. THE CALENDAR FIX: Force re-draw to prevent "Blank Void"
...
function buildRegistry(tabId) {
    ...
    if (tabId === 'trucks') {
        ...
        headers = ['Date', 'Driver Name', 'Unit ID', 'Route', 'Sync'];
        rows = fleetData.logs.map(l => [l.date, l.driver, l.truckId, l.route || 'Local', 'LIVE']);
    } else if (tabId === 'clients') {
        headers = ['Partner', 'Contact', 'Location', 'Base Rate', 'Sync'];
